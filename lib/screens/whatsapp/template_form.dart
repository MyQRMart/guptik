...
import 'package:guptik/utils/whastapp%20templates/Constants.dart';
import 'package:guptik/utils/whastapp%20templates/helpers.dart';
import 'package:guptik/widgets/whatsapp/button_editor.dart';
  ...
  void _showErrorDialog(String errorMessage) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SingleChildScrollView(child: SelectableText(friendlyMessage)),
  ...
  Future<void> _submit() async {
    ...
    try {
      if (_headerType != 'NONE' &&
        ...
        if (_headerType == 'IMAGE') {
          headerHandle = await _whatsapp.uploadImageForTemplateHeader(
            _headerFile!,
          );
          headerMediaId = await _whatsapp.uploadMediaForMessage(
            _headerFile!,
            'whatsapp',
          );
        } else if (_headerType == 'VIDEO') {
          headerHandle = await _whatsapp.uploadVideoForTemplateHeader(
            _headerFile!,
          );
          headerMediaId = await _whatsapp.uploadMediaForMessage(
            _headerFile!,
          ...
          headerHandle = await _whatsapp.uploadDocumentForTemplateHeader(
            _headerFile!,
          );
          headerMediaId = await _whatsapp.uploadMediaForMessage(
            _headerFile!,
      ...
      if (_footerController.text.isNotEmpty) {
        components.add({'type': 'FOOTER', 'text': _footerController.text});
      }

      if (_buttons.isNotEmpty) {
        components.add({'type': 'BUTTONS', 'buttons': _buttons});
      }
      ...
      final newTemplate = WhatsAppTemplate(
        ...
        variables: variables.asMap().map(
          (i, v) => MapEntry(v, _sampleVariables[v]),
        ),
        buttons: _buttons,
        status: actualStatus,
        createdAt: DateTime.now(),
        metaBusinessAccountId: _supabase
            .getCurrentUserId(), // You might want to store this properly
        whatsappPhoneNumberId: _supabase
            .getCurrentUserId(), // You might want to store this properly
