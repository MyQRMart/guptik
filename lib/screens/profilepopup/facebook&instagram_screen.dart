...
class _FacebookAndInstagramScreenState
  ...
  Future<void> _loadSavedKeys() async {
    try {
      ...
      if (mounted && response != null) {
        final hasData = response['facebook_user_access_token'] != null ||
  ...
  Future<void> _saveSettingsToSupabase() async {
    if (_userToken == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please connect your Facebook account first.')),
      );
      return;
    }
  ...
  Future<void> _clearSocialSettings(int index) async {
    try {
      ...
      await Supabase.instance.client.from('user_api_settings').update({
  ...
  Future<void> _fetchPagesAndInstagram(String userToken) async {
    try {
      ...
      setState(() {
        _pages = pages.map<Map<String, dynamic>>((p) => {
          'id': p['id'],
          'name': p['name'],
  ...
  Future<void> _fetchBusinesses(String userToken) async {
    try {
      ...
      setState(() {
        _businesses = biz.map<Map<String, dynamic>>((b) => {
          'id': b['id'],
          'name': b['name'],
        }).toList();
  ...
  // ─────────────────────────────────────────────────────────────────────────
  Future<void> _fetchWhatsAppNumbers(String userToken, String businessId) async {
    try {
      ...
      setState(() {
        _wabaPhoneNumbers = phones.map<Map<String, dynamic>>((p) => {
          'id': p['id'],                               // This is meta_wa_phone_number_id
          'display_phone_number': p['display_phone_number'], // This is mobile_number
      ...
      debugPrint('✅ WhatsApp phone numbers fetched: ${_wabaPhoneNumbers.length}');
    } catch (e) {
        ...
        child: Column(
          ...
          children: [
            ...
            if (account['facebook_token'] != null) ...[
              ...
              _buildInfoRow("Account ID", account['facebook_account_id'] ?? '—'),
